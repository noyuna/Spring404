...
function App() {
  ...
  const analyzeRouteSafety = (path, allReviews) => {
    ...
    const sum = nearReviews.reduce((acc, review) => {
      return acc + Number(review.ai_score || 0);
    }, 0);

    return {
      safetyScore: Number((sum / nearReviews.length).toFixed(2)),
      nearReviews,
    };
  };
      ...
      directionsService.route(
        ...
        (result, status) => {
          ...
          const candidates = result.routes.map((route, index) => {
            ...
            const safety = analyzeRouteSafety(path, allReviews);
            ...
            return {
              ...
              nearReviews: safety.nearReviews,
              path,
            };
          });
              ...
              >
                ...
                <div style={scoreBoxStyle}>
                  ...
                  <div
                    style={{
                      ...scoreValueStyle,
                      color: getSafetyColor(Number(getAiAverage(reviews))),
                    }}
                  >
                    {getAiAverage(reviews)} / 5
                  </div>
                </div>
              </div>
